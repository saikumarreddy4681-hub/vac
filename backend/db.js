...
// We export an object that has the `query` method.
// If we are in production or MySQL connection fails, we redirect queries to Mongoose.
...
if (!useMongooseFallback) {
    try {
        ...
        connection.connect(error => {
            if (error) {
                console.warn('[DB] Local MySQL database not running. Falling back to MongoDB/Mongoose for vehicles.');
        ...
        console.warn('[DB] Failed to initialize MySQL. Falling back to MongoDB/Mongoose.', err.message);
        useMongooseFallback = true;
    }
}

// Require Vehicle schema dynamically to avoid initialization race conditions
